...
export default function slackin({
  ...
  // must haves
  if (!token) throw new Error('Must provide a `slack api token`.');
  if (!org) throw new Error('Must provide an `slack org`.');
  if (!mailchimp_api_key) throw new Error('Must provide an `mailchimp api key`.');
  if (!mailchimp_datacenter) throw new Error('Must provide an `mailchimp datacenter`.');
  if (!mailchimp_list_id) throw new Error('Must provide an `mailchimp list id`.');
  ...
  // fetch data
  let slack = new Slack({ token, interval, org, mailchimp_datacenter, mailchimp_api_key, mailchimp_list_id });
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    invite({ token, org, email, mailchimp_datacenter, mailchimp_api_key, mailchimp_list_id, channel: chanId }, err => {
      if (err) {
        return res
        .status(400)
