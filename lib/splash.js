...
export default function splash({ name, org, logo, active, total, channels, iframe }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' Geekettes online now of ',
          dom('b.total', total), ' registered.'
        ]
        : [dom('b.total', total), ' Geekettes are registered so far.']
    ),
    dom('form',
    ...
    dom('p.signin',
      'To join this Slack team, you must be a member of the ', dom('b', name), ' community and subscribed to the email list. To become a ', dom('b', name),
      ' member, go to ',
      dom('a href=http://www.geekettes.io/membership target=_blank', 'geekettes.io/membership'), ' and sign up.'
    ),
    !iframe && dom('footer',
      'powered by ',
