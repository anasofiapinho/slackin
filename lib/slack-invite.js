...
export default function invite({ org, token, email, mailchimp_datacenter, mailchimp_api_key, mailchimp_list_id, channel }, fn){
  ...
  request
    ...
    .end((err, res) => {
      ...
      if (404 == res.status) {
        fn(new Error('You are not a member of the Geekettes community. Be sure to sign up to be a member <a href="http://www.geekettes.io/membership">here</a> first and then confirm your email subscription.'));
        return;
      }
      if (200 != res.status) {
      ...
      if (res.body.status == "pending") {
        fn(new Error('You have requested membership to the Geekettes community, however you have not confirmed your email subscription. You must have be subscribed to the email list to gain access to the Slack community. Check your email for a confirmation or sign up <a href="http://www.geekettes.io/membership">here</a>.'));
        return;
      }

      if (res.body.status == "unsubscribed" || res.body.status == "cleaned") {
        fn(new Error('Your email has been removed from the Geekettes email list (because it bounced or you unsubscribed). You must have be subscribed to the email list to gain access to the Slack community. You can sign up <a href="http://www.geekettes.io/membership">here</a>.'));
        return;
      }
